...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # draw a next ball on mouse motion
            mouse_x = event.pos[0]
            mouse_y = event.pos[1]
    ...
    if not game_over:
            ...
            for j in range(i+1, len(balls)):
                ...
                if distance < balls[i][5] + balls[j][5]:
                    ...
                    if balls[i][4] == balls[j][4]:
                        ...
                        # when two balls collide and are removed, and the score is doubled
                        if color_index == len(color_order) - 1:
                            score += score_order[color_index] * 2
                        if color_index + 1 < len(color_order):
                            # add a new ball with the next color
                        ...
                        break
                    # if they touch but not of same colors
                    else:
        ...
        for ball in balls:
            ...
            ball[0] += ball[3]
            # check if the ball is out of box horizontally
            if ball[0] < box_left:
                ball[0] = box_left + 1
                ball[3] *= -0.5
